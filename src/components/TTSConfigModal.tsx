...
export default function TTSConfigModal({ isOpen, onClose, bookLanguage }: TTSConfigModalProps) {
  ...
  const [enabled, setEnabled] = useState(ttsConfig.enabled);
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTestResult({
          success: false,
          message: `连接失败: ${response.status} - ${error.slice(0, 100)}`
        });
      }
    } catch (error) {
      setTestResult({
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">大模型 TTS 配置</h2>
          <button
            onClick={onClose}
            ...
            <>
              ...
              <div>
                ...
                {bookLanguage && (
                  <p className="text-xs text-blue-500 mb-1">
                    📖 当前文章语言: {LANGUAGE_MAP[bookLanguage]?.label ?? '中文'}（已自动同步）
                  </p>
                )}
                <select
                  value={language}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="例如：缓慢而温柔地朗读，适合睡前故事"
                  rows={2}
                  className="w-full p-2 border rounded-lg text-sm"
                />
                <p className="text-xs text-gray-400 mt-1">
              ...
              {showAdvanced && (
                <div className="space-y-3 bg-gray-50 p-3 rounded-lg">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => setEnableAudioCache(!enableAudioCache)}
                      className={`w-10 h-5 rounded-full transition-colors ${
                        enableAudioCache ? 'bg-blue-500' : 'bg-gray-300'
                      }`}
                    >
                      <div
                        className={`w-4 h-4 bg-white rounded-full transition-transform ${
                          enableAudioCache ? 'translate-x-5' : 'translate-x-0.5'
                        }`}
                      />
                    </button>
                  </div>
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            onClick={handleSave}
            className="flex-1 p-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition"
          >
            保存
          </button>
        </div>
